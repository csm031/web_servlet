...
- DBCP 매니저가 어느정도의 연결객체를 확보해 놓고 있다가 사용자의 요청이 들어오면 연결객체를 지원해주고
 작업이 끝나면 연결객체를 DBCP 매니저에게 반환하도록 함
 - 사용자의 요청을 처리하기 이해 매번 드라이버를 로드하고 연결객체를 생성해서 사용해야 하는 작업을 하지 않아도 됨
...
public class DBCP {
    ...
    public DBCP() {
        try {
            ...
            //JNDI 를 이용하기 위해 InitialContext 클래스로 Context 객체를 생성함
            Context ctx = (Context) initCtx.lookup("java:comp/env");
            //Context 객체의 lookup("java:comp/env")를 이용해서 server.xml 에 대한 Context 객체 얻음
            DataSource source = (DataSource) ctx.lookup("oracle_dbcp");
            //Context 객체의 lookup("oracle_dbcp")로 DBCP 를 지원하는 DataSource 객체 얻음

            //DataSource 객체에서 연결객체 얻기
            conn = source.getConnection();

            //DataSource 객체에서 연결 객체 얻기
